...
/work/scripts/test-single.sh "/work/demo-quarkus/target/demo-ce -Xmn8M -Xmx8M" $DEMO_URL quarkus-native-simple-ce "Quarkus (JAX-RS) via GraalVM Native Image (21.1.0 CE)"
sleep 2

# run advanced quarkus native image test
/work/scripts/test-single.sh "/work/demo-quarkus-jpa/target/demo-ce -Xmn8M -Xmx8M" $DEMO_URL quarkus-native-advanced-ce "Quarkus (JAX-RS + JPA) via GraalVM Native Image (21.1.0 CE)"
sleep 2

# run simple spring-boot native image test
/work/scripts/test-single.sh "/work/demo-spring-boot/target/demo-ce -Xmn16M -Xmx16M" $DEMO_URL spring-boot-native-simple-ce "Spring Boot (REST) via GraalVM Native Image (21.1.0 CE)"
sleep 2

# run advanced spring-boot native image test
/work/scripts/test-single.sh "/work/demo-spring-boot-jpa/target/demo-ce -Xmn16M -Xmx16M" $DEMO_URL spring-boot-native-advanced-ce "Spring Boot (REST + JPA) via GraalVM Native Image (21.1.0 CE)"
sleep 2
...
jabba ls | while read CURRENT_JAVA; do
    ...
    /work/scripts/test-single.sh "java -Xmn8M -Xmx32M -jar /work/demo-quarkus/target/quarkus-app/quarkus-run.jar" $DEMO_URL quarkus-java-simple-$CURRENT_JAVA "Quarkus (JAX-RS) via [$CURRENT_JAVA]"
    sleep 2

    # run advanced quarkus-java test
    /work/scripts/test-single.sh "java -Xmn16M -Xmx32M -jar /work/demo-quarkus-jpa/target/quarkus-app/quarkus-run.jar" $DEMO_URL quarkus-java-advanced-$CURRENT_JAVA "Quarkus (JAX-RS + JPA) via [$CURRENT_JAVA]"
    sleep 2
done